...
describe('flattenPathData', () => {
	...
	it("reads the dot a tap leaves — paper writes it 'v0'", () => {
		// A press that never moved. The pencil repeats the point so that the subpath is
		// a zero-length *line* rather than a lone moveto, which is the only form SVG
		// and canvas agree to stroke; here that has to survive as two points, because
		// one is a stroke `svgPages` drops and `rasterise` never enters.
		expect(flattenPathData('M100,50v0')).toEqual([100, 50, 100, 50]);
	});
...
describe('svgPages', () => {
	...
	it('keeps a dot, which is two points and the shortest stroke there is', () => {
		const pages = svgPages(file(page(['M100,50v0'])));
		expect(pages[0]).toHaveLength(1);
		expect(pages[0][0].points).toEqual([100, 50, 100, 50]);
	});
...
describe('rasterise', () => {
	...
	it('draws a zero-length stroke as a round dot, which is what a tap leaves', () => {
		const cover = blank();
		rasterise([{ points: [50, 50, 50, 50], width: 20 }], cover);

		expect(at(cover, 50, 50)).toBe(1);
		expect(at(cover, 55, 50)).toBe(1); // still within the ten-unit radius
		expect(at(cover, 62, 50)).toBe(0); // and nothing beyond it
		expect(at(cover, 41, 41)).toBe(0); // round, so the corner of the box stays empty
	});
...
/*
 * A GIF is the size of the flipbook it is of, and there are two sizes now.
 *
 * This was the one place the old fixed 640x360 buffer went silently wrong rather than
 * loudly: a square flipbook rasterised into a 16:9 frame keeps its top 360 rows and
 * loses the rest, so `/f/:id.gif` would have served everybody else's site a drawing with
 * its bottom 44% cut off — on the one surface nobody here ever looks at.
 */
describe('flipbookGif on a square flipbook', () => {
	const SQUARE_ROOT = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg"' +
		' width="640" height="640" viewBox="0,0,640,640">';

	const square = (...pages) => `${SQUARE_ROOT}${SYSTEM}${pages.join('')}</svg>`;

	// A stroke well below y=360, which is exactly what a 16:9 buffer would have dropped.
	const artwork = square(page(['M50,50l200,100', 'M100,500l300,100']), page(['M60,600l200,-100']));

	it('is the size the artwork says it is, not the legacy one', () => {
		const gif = decodeGif(flipbookGif(artwork, 'svg'));

		expect(gif.width).toBe(640);
		expect(gif.height).toBe(640);
		expect(gif.frames).toHaveLength(2);

		for (const frame of gif.frames) {
			expect(frame.width).toBe(640);
			expect(frame.height).toBe(640);
			expect(frame.indices).toHaveLength(640 * 640);
		}
	});

	it('keeps the ink that lives below the legacy page’s bottom edge', () => {
		const gif = decodeGif(flipbookGif(artwork, 'svg'));
		const first = gif.frames[0].indices;

		// Somewhere on `M100,500l300,100` — past y=360, so this is the assertion that
		// would have failed before the frame took its size from the file.
		const inked = first.some((value, i) => Math.floor(i / 640) > 360 && value !== 0);
		expect(inked).toBe(true);
	});

	it('still writes the legacy size for artwork that states none', () => {
		// The archive, and the rule the whole change rests on.
		const bare = `<svg xmlns="http://www.w3.org/2000/svg">${SYSTEM}${page(['M10,10l100,50'])}</svg>`;
		const gif = decodeGif(flipbookGif(bare, 'svg'));

		expect(gif.width).toBe(GIF_WIDTH);
		expect(gif.height).toBe(GIF_HEIGHT);
	});

	it('rasterises a 2012 flipbook at the legacy size, which is the only one it can be', () => {
		const legacy = JSON.stringify({
			layers: [{}, {}, {}, { children: [{ segments: [{ x: 10, y: 10 }, { x: 100, y: 300 }] }] }]
		});
		const gif = decodeGif(flipbookGif(legacy, 'legacy-json'));
