...
import { flipbookGif } from './gif.js';
...
export async function handleApi(req, res) {
	...
	try {
		...
		const match = /^\/api\/flipbooks\/([^/]+)(?:\/(data|thumbnail|thumbnail\.svg|gif))?$/.exec(path);
		if (match) {
			if (!isRead(req)) throw new HttpError(405, 'Method not allowed');
			...
			if (match[2] === 'gif') return await sendGif(res, id);
...
async function saveFlipbook(req, res) {
	...
	const id = await createFlipbook({
		...
		// Absent from every save this branch makes: the create page's "contains adult
		// stuff" checkbox is gone and flagging is an admin action now. Still read, because
		// `time-capsule` posts to this same endpoint and still has the checkbox — and
		// absent reads as false, which is what a new save should be either way.
...
/**
 * The whole flipbook as an animated GIF — what `/f/:id.gif` serves.
 *
 * The URL a reader sees is `/f/:id.gif`, rewritten here by vercel.json (and by the
 * dev server's middleware) so that this stays one more thing `/api/flipbooks/:id/…`
 * can be asked for rather than a route of its own. It answers with bytes and nothing
 * else: no HTML and no application, because everything worth pasting a GIF link into
 * — Slack, Discord, an `<img src>` — issues a GET and expects an image back.
 *
 * **Rendered per request rather than stored**, and cached forever by the CDN on the
 * strength of the artwork being immutable, exactly as `/data` and the thumbnails are.
 * Storing it was measured and rejected: at a ~0.4 MB mean across 594 rows that is
 * another ~240 MB of Neon against a 0.5 GB tier holding 77 MB today, for bytes already
 * LZW-compressed and so incompressible in `bytea`. What it costs instead is about
 * 200ms of function time per flipbook per edge region, once.
 *
 * **No view is counted.** `getFlipbookData` doesn't count one, which is the same
 * answer `/data` gets and the right one here: an `<img>` on somebody else's page
 * pulling this a thousand times is not a thousand people watching a flipbook.
 *
 * NSFW rows are served, as `/data` serves them. That flag has only ever kept a
 * flipbook out of the browse grids, not off its own URL.
 */
async function sendGif(res, id) {
	const row = await getFlipbookData(id);
	if (!row) throw new HttpError(404, 'Not found');

	// The one place the API decompresses anything. Everything else is a passthrough of
	// what is stored, and can be; a GIF is made out of the artwork rather than being
	// the artwork, so it has to be read.
	const artwork = row.data_br
		? await brotliDecompress(row.data_br)
		: await gunzip(row.data_gz);

	const gif = flipbookGif(artwork.toString('utf8'), row.format);
	// Null means there were no pages in it — a file this can't make an animation out
	// of. 404 rather than a blank frame, which would look like a working GIF of an
	// empty flipbook.
	if (!gif) throw new HttpError(404, 'Not found');

	res.writeHead(200, {
		'Content-Type': 'image/gif',
		'Content-Length': gif.length,
		'Cache-Control': IMMUTABLE,
		// So that saving it lands as `a2715.gif` rather than as whatever the browser
		// makes of the last path segment. `inline`, because it is also being displayed.
		'Content-Disposition': `inline; filename="${id}.gif"`
	});
	res.end(gif);
}
