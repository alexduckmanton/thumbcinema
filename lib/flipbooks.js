...
import { coverSvg, pageSize } from './thumbnail.js';
...
const OPTIONAL_COLUMNS = {
	...
	// Both are written and read together and always will be, so one entry stands for
	// the pair: standing down on one and not the other would store a width with no
	// height, which is worse than storing neither.
	width: 'Every flipbook will be laid out as 640x360 until it exists.'
};
...
export async function listFlipbooks({ view = 'featured', cursor = null, limit = 24, includeNsfw = false, remixOf = null } = {}) {
	...
	const { rows } = await queryColumnAware((has) => {
		...
		return [
			`SELECT id, title, source, format, featured, nsfw, created_at,
			        ${has('width') ? 'width, height' : 'NULL AS width, NULL AS height'},
	...
	const items = rows.map((row) => ({
		...
		// What shape the card should be. Null before the migration, and the grid falls
		// back to 640x360 — which every row that predates the column is.
		width: row.width,
		height: row.height
	}));
...
export async function getFlipbook(id, { countView = false } = {}) {
	...
	const { rows } = await queryColumnAware((has) => {
		...
		// The shape of the page, so the playback page's sheet of paper is the right
		// shape under its placeholder rather than becoming it when the artwork lands.
		const size = has('width') ? 'width, height' : 'NULL AS width, NULL AS height';
		...
		return [
			countView
				...
				RETURNING id, title, description, format, source, views, featured, nsfw,
				          created_at, ${size}, ${svg}, ${parent}, ${root}`
	...
	return {
		...
		thumbnail_url: `/api/flipbooks/${row.id}/thumbnail`,
		thumbnail_svg_url: row.has_thumbnail_svg ? `/api/flipbooks/${row.id}/thumbnail.svg` : null,
		// The shape of a page. Null before the migration; the page falls back to 640x360
		// and the artwork restates it either way. See `pageSizeFromSvg`.
		width: row.width,
		height: row.height,
...
export async function createFlipbook({ title, description, svg, thumbnailDataUrl, cover = null, nsfw = false, remixOf = null }) {
	...
	// The shape the artwork itself states, which is the only shape it can be: these
	// coordinates are in whatever space the exporting project was in, and the viewBox
	// is that project saying so. Not taken from the form — a client is free to be
	// wrong about its own drawing, and a card laid out to a claim rather than to the
	// artwork would hold a picture of the wrong shape.
	...
	await queryColumnAware((has) => {
		...
		// Both together or neither: a width with no height is a shape nothing can use,
		// and the pair is read back the same way. Taken from the artwork rather than
		// from the client, so a save can't claim a shape its coordinates aren't in.
		if (has('width')) {
