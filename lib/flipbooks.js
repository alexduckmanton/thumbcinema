...
export async function createFlipbook({ title, description, svg, thumbnailDataUrl, nsfw = false }) {
	...
	// Both, on every save. The gzip is what `time-capsule` serves from and what any
	// client that won't take brotli gets; the brotli is what everyone else gets. They
	// compress in parallel because the brotli is the slow one and there is a reader
	// waiting on this request for their permalink.
	const [dataGz, dataBr] = await Promise.all([gzip(raw, { level: 9 }), brotli(raw)]);
	const id = newId();
