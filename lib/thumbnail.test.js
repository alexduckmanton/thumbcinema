...
import { busiestPage, coverSvg, pageSize } from './thumbnail.js';
...
/*
 * What shape a row is, which is what gets written into the width/height columns.
 *
 * The server's half of a pair: `pageSizeFromSvg` in `src/flipbook/engine/formats.ts` is
 * the client's, and they have to give the same answer to the same bytes. This one
 * decides what a card is laid out as; that one decides what the drawing on it is scaled
 * by. Disagreeing would be a tile of one shape holding a picture of another.
 */
describe('pageSize', () => {
	it('reads the viewBox paper 0.12 writes', () => {
		expect(pageSize(`${ROOT}<g/></svg>`)).toEqual({ width: 640, height: 360 });
		expect(pageSize('<svg viewBox="0,0,640,640"><g/></svg>')).toEqual({ width: 640, height: 640 });
	});

	it('is 640x360 for the archive, which states nothing at all', () => {
		// The same root `rootAttributes` exists to repair, asked the same question.
		expect(pageSize(`${ARCHIVE_ROOT}<g/></svg>`)).toEqual({ width: 640, height: 360 });
	});

	it('takes either separator, because paper has written both', () => {
		expect(pageSize('<svg viewBox="0 0 640 640"/>')).toEqual({ width: 640, height: 640 });
		expect(pageSize('<svg viewBox=" 0 , 0 , 640 , 640 "/>')).toEqual({ width: 640, height: 640 });
	});

	it('ignores width and height when a viewBox says otherwise', () => {
		expect(pageSize('<svg width="320" height="180" viewBox="0,0,640,640"/>')).toEqual({
			width: 640,
			height: 640
		});
	});
	...
	it('falls back rather than throwing on anything malformed', () => {
		// This runs inside a save. A wrong shape is a card that looks odd; a throw here
		// is somebody's drawing refused.
		for (const bad of ['', null, undefined, 'not svg', '<svg viewBox=""/>', '<svg viewBox="0 0 640"/>',
			'<svg viewBox="0 0 0 360"/>', '<svg viewBox="a b c d"/>']) {
			expect(pageSize(bad)).toEqual({ width: 640, height: 360 });
		}
	});

	it('rounds to whole units, because these go into INTEGER columns', () => {
		expect(pageSize('<svg viewBox="0,0,639.6,360.2"/>')).toEqual({ width: 640, height: 360 });
	});
});
