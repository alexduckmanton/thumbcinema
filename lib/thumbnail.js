 ...
 *
 * The same pair as `LEGACY_PAGE_SIZE` in `src/flipbook/engine/constants.ts`
 ...
 *
 * The shape of everything drawn between 2012 and 2026, and so the answer for any
 * artwork that doesn't say what shape it is — see `rootAttributes` and `pageSize`.
 */
const LEGACY_WIDTH = 640;
const LEGACY_HEIGHT = 360;
...
/**
 * The shape of a page, read off the artwork's own root element.
 *
 * The server's copy of `pageSizeFromSvg` in `src/flipbook/engine/formats.ts`, and it
 * has to give the same answer to the same bytes: this is what gets written into the
 * `width`/`height` columns at save time, and that is what the gallery lays a card out
 * against while the front end sizes the drawing from the file. The two disagreeing
 * would be a card of one shape holding a drawing of another.
 *
 * **No viewBox means 640x360.** paper 0.8 wrote no viewBox, no width and no height, so
 * every one of the archive rows is silent — and every one of them is that shape. paper
 * 0.12 writes all three. See the much longer note on `rootAttributes`, which is the
 * same fact put to a different use.
 *
 * The width/height attributes are ignored when a viewBox is present: those are a
 * display size, and it is the viewBox that states the space the coordinates are in.
 */
export function pageSize(svgText) {
	const match = /<svg\b[^>]*\sviewBox\s*=\s*["']([^"']+)["']/i.exec(String(svgText || ''));
	if (!match) return { width: LEGACY_WIDTH, height: LEGACY_HEIGHT };

	// "0,0,640,360" and "0 0 640 360" are both written by paper depending on version.
	const parts = match[1].trim().split(/[\s,]+/).map(Number);
	if (parts.length !== 4) return { width: LEGACY_WIDTH, height: LEGACY_HEIGHT };

	const [, , width, height] = parts;
	if (!(width > 0) || !(height > 0)) return { width: LEGACY_WIDTH, height: LEGACY_HEIGHT };

	// Whole units: these go into INTEGER columns, and paper has never written a
	// fractional view size. Rounding rather than refusing keeps a save from failing
	// over a number nothing has ever produced.
...
function rootAttributes(attributes) {
	...
	const sized = /\s(?:width|height)\s*=/i.test(` ${namespaced}`)
		? namespaced
		: ` width="${LEGACY_WIDTH}" height="${LEGACY_HEIGHT}"${namespaced}`;

	return ` viewBox="0,0,${LEGACY_WIDTH},${LEGACY_HEIGHT}"${sized}`;
}
