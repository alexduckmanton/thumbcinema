...
import { pageGroups, tags } from './thumbnail.js';

/**
 * The project's coordinate space, which is 640x360 whatever the canvas was shown at.
 *
 * `Scene.pinCoordinates()` is what holds that true, so the numbers in the file are
 * always in this space and the GIF is always this size. Stated rather than read off
 * the artwork for the same reason the engine and the preview renderer state it: the
 * whole 2013-2015 archive was written with no viewBox at all.
 */
...
export function flipbookGif(text, format) {
	...
	writeHeader(out, pages.length);

	// One coverage buffer and one index buffer for the whole flipbook, cleared between
	// pages. A page is 230,400 pixels and there may be two hundred of them.
	const cover = new Float32Array(GIF_WIDTH * GIF_HEIGHT);
	const indices = new Uint8Array(GIF_WIDTH * GIF_HEIGHT);
	const lzw = new Lzw();
	...
	for (let i = 0; i < pages.length; i++) {
		...
		writeFrame(out, indices, delayFor(i), lzw);
	}
 ...
 */
export function rasterise(strokes, cover) {
	for (const stroke of strokes) {
		...
		for (let i = 0; i + 3 < points.length; i += 2) {
			...
			const right = Math.min(GIF_WIDTH - 1, Math.ceil(Math.max(ax, bx) + radius + 1));
			const top = Math.max(0, Math.floor(Math.min(ay, by) - radius - 1));
			const bottom = Math.min(GIF_HEIGHT - 1, Math.ceil(Math.max(ay, by) + radius + 1));
...
function writeHeader(out, frames) {
	out.text('GIF89a');
	out.u16(GIF_WIDTH);
	out.u16(GIF_HEIGHT);
 ...
 */
function writeFrame(out, indices, delay, lzw) {
