...
`GET /api/flipbooks/:id[/data|/thumbnail|/thumbnail.svg|/gif]`,
`PATCH /api/admin/flipbooks/:id`. See `src/lib/api.ts`, which is the only place the
front end knows about any of them — `/gif` is the exception, being for other people's
pages rather than for this one. See **A flipbook as a GIF** below.
...
- **You can trace over a photograph.** A sixth disc in the phone's footer takes a picture
  ...
  wholly outside the artwork — see **Tracing over a photograph** below.
...
- **The card moved to `src/flipbook/card/` and the list is `lazy()`.** See **Code
  splitting** above: the card is shared with the gallery, and importing it plainly on the
