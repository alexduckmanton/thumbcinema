...
export function usePageReorder(
	...
	const onPointerDown = (event: React.PointerEvent<HTMLElement>) => {
		...
		// How far one page of travel is, measured off the sheet you are holding.
		//
		// It used to be the engine's, reported by the page strip, which was a row of
		// full-size copies of the drawing and so knew the pitch by measuring its own
		// layout. There is no strip, so the number comes from the one thing left that
		// knows how wide a page is drawn: the paper itself. The 20 is the gap the strip's
		// pages used to stand apart by, kept so a drag feels the same length it did.
		const pitch = (bookRef.current?.offsetWidth ?? 0) + 20
		if (pitch <= 20 || pages < 2) return
